-- ============================================================
-- H-AUTOS NIGERIA — COMPLETE SUPABASE SCHEMA v2
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  seller_active       BOOLEAN DEFAULT false,    -- true = paid & admin-approved seller
  seller_expiry       TIMESTAMPTZ,             -- when seller subscription ends
...
CREATE POLICY "profiles_update" ON public.profiles FOR UPDATE USING (auth.uid() = id OR EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = true));

-- 2. STORES (Car Stands / Dealerships)
CREATE TABLE IF NOT EXISTS public.stores (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  owner_id          UUID REFERENCES public.profiles(id) ON DELETE SET NULL,
  owner_email       TEXT,
  name              TEXT NOT NULL,
  rc_number         TEXT,               -- CAC RC or BN number
  logo_url          TEXT,
  banner_url        TEXT,
  address           TEXT,
  city              TEXT,
  ...
  is_verified       BOOLEAN DEFAULT false,   -- true after admin verifies RC
  created_at        TIMESTAMPTZ DEFAULT NOW(),
  updated_at        TIMESTAMPTZ DEFAULT NOW()
);
...
-- 4. SELLER SUBSCRIPTION REQUESTS
CREATE TABLE IF NOT EXISTS public.seller_requests (
  ...
  phone           TEXT,
  whatsapp        TEXT,
  payment_method  TEXT,    -- 'naira' | 'usdt'
  payment_ref     TEXT,    -- transaction reference
  amount_paid     NUMERIC,
  currency        TEXT,    -- 'NGN' | 'USDT'
  status          TEXT DEFAULT 'pending', -- 'pending' | 'approved' | 'rejected'
...
CREATE POLICY "seller_req_select" ON public.seller_requests FOR SELECT USING (user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = true));
CREATE POLICY "seller_req_insert" ON public.seller_requests FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "seller_req_update" ON public.seller_requests FOR UPDATE USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = true));

-- 5. ENQUIRIES / ORDERS (all flow to admin WhatsApp +234 814 673 0044)
...
CREATE POLICY "enquiries_select" ON public.enquiries FOR SELECT USING (user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = true));
...
CREATE POLICY "newsletter_select" ON public.newsletter_subscribers FOR SELECT USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND is_admin = true));
...
-- 1. Bucket name: "cars"   → Public: ON
-- 2. Bucket name: "stores" → Public: ON
-- Add INSERT policy for each: (auth.role() = 'authenticated')

-- ─── MAKE YOURSELF ADMIN ─────────────────────────────────────────────────────
-- After your first Google sign-in, run this:
