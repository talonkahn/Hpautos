...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* ── Hero ─────────────────────────────────────────────────────── */}
      <section className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-hidden min-h-[80vh] sm:min-h-[88vh] flex items-center">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 sm:py-24 lg:py-28 w-full text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <Badge className="mb-4 sm:mb-6 bg-amber-500/10 text-amber-400 border-amber-500/20 px-4 sm:px-5 py-1.5 sm:py-2 text-xs sm:text-sm">
            ...
            <p className="text-base sm:text-xl text-slate-300 mb-6 sm:mb-10 max-w-2xl mx-auto px-2">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }}
            className="bg-white/10 backdrop-blur-xl rounded-2xl p-1.5 sm:p-2 max-w-4xl mx-auto mb-6 sm:mb-10">
            <div className="flex gap-2 flex-wrap md:flex-nowrap">
              {/* Country picker */}
              <Select value={searchCountry || 'all'} onValueChange={v => setSearchCountry(v === 'all' ? '' : v)}>
                <SelectTrigger className="bg-slate-900/80 border border-amber-500/30 hover:border-amber-400/60 h-12 sm:h-14 md:w-52 w-full rounded-xl text-amber-50 font-medium shrink-0 backdrop-blur-sm transition-colors text-sm sm:text-base">
                  <SelectValue placeholder="🌍 All countries">
                    {searchCountry && (() => {
                      const c = COUNTRIES.find(x => x.code === searchCountry);
                      return c ? <span>{c.flag} {c.name}</span> : null;
                    })()}
                    {!searchCountry && <span>🌍 All countries</span>}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent className="bg-slate-900 border border-amber-500/30 text-amber-50">
                  <SelectItem value="all" className="focus:bg-amber-500/20 focus:text-amber-100">
                    🌍 All countries
                  </SelectItem>
                  {COUNTRIES.map(c => (
                    <SelectItem key={c.code} value={c.code} className="focus:bg-amber-500/20 focus:text-amber-100">
                      {c.flag} {c.name}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className="pl-11 sm:pl-12 h-12 sm:h-14 text-sm sm:text-base bg-white border-0 rounded-xl"
                />
              </div>
              <Button onClick={handleSearch} className="h-12 sm:h-14 px-6 sm:px-8 bg-amber-500 hover:bg-amber-600 text-white font-semibold rounded-xl text-sm sm:text-base shrink-0 w-full md:w-auto">
                Search
              </Button>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7, delay: 0.4 }}
            ...
            {COUNTRIES.map(c => (
              <button key={c.code} onClick={() => navigate(`/Browse?country=${c.code}`)}
                className="px-3 py-1.5 bg-amber-500/80 hover:bg-amber-500 text-white rounded-full text-sm font-medium transition-all shadow-sm hover:shadow-md">
                {c.flag} {c.name}
              </button>
            ))}
          </motion.div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          ...
          {latestCars.length === 0
            ...
            : <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{latestCars.map(car => <CarCard key={car.id} car={car} />)}</div>
          }
        </div>
      </section>
