...
import {
  getAllCarsAdmin, getAllUsersAdmin, getAllStoresAdmin, getSellerRequests, getEnquiries,
  updateCar, deleteCar, getAdminStats, createCar, uploadFile, upsertProfile,
  sendEmailNotification, getActiveAds, createAd, updateAd, deleteAd,
  updateSellerRequest, updateStore, deleteStore, ADMIN_WHATSAPP, ADMIN_EMAIL, SELLER_DURATION_DAYS,
  createStore
} from '@/lib/supabase';
...
import { Car, Users, CheckCircle2, XCircle, Clock, Eye, Search, Star, Shield, Plus, Trash2, Edit, MessageSquare, Crown, Megaphone, Store, UserCheck, TrendingUp, Phone, Image as ImageIcon, Loader2, X } from 'lucide-react';
import { formatPrice, CAR_MAKES, NIGERIAN_STATES } from '@/components/shared/NigerianStates';
...
export default function AdminDashboard() {
  ...
  const [listForm, setListForm] = useState({
    ...
    transmission:'automatic',mileage:'',title:'',store_id:''
  });
  ...
  const loadAll = async () => {
    try {
      const [s, c, u, st, sr, enq, a] = await Promise.all([
        getAdminStats(), getAllCarsAdmin(), getAllUsersAdmin(), getAllStoresAdmin(),
        getSellerRequests(), getEnquiries(), getActiveAds()
      ]);
      setStats(s); setCars(c); setUsers(u); setStores(st); setSellerReqs(sr); setEnquiries(enq); setAds(a);
  ...
  const approveCar = async (car) => {
    ...
    await sendEmailNotification({ to:car.seller_email||car.profiles?.email, subject:`✅ Your listing is live – ${car.title}`, html:`<h2>${car.title} is now live on H-Autos!</h2><p>Buyers can now contact you at ${car.seller_whatsapp}. Log in to track views.</p>` });
  ...
  const approveSellerReq = async (req) => {
    ...
    await sendEmailNotification({ to:req.profiles?.email, subject:'🎉 Seller Access Approved – H-Autos', html:`<h2>You're now an active seller!</h2><p>Your ${SELLER_DURATION_DAYS}-day seller subscription is active until ${expiry.toLocaleDateString()}.</p><p>WhatsApp admin to submit car listings: wa.me/${ADMIN_WHATSAPP}</p>` });
    const waMsg = `✅ Seller approved!\nName: ${req.full_name}\nActive for ${SELLER_DURATION_DAYS} days.\nThey can now submit cars via WhatsApp.`;
  ...
  const approveStore = async (store) => {
    ...
    await sendEmailNotification({ to:store.profiles?.email||store.email, subject:`🏪 ${store.name} is now live on H-Autos!`, html:`<h2>Your store is approved!</h2><p>${store.name} is now live on H-Autos. Buyers can find your store at H-Autos Nigeria.</p>` });
  ...
  const upgradeUser = async (u) => {
    await upsertProfile({ id:u.id, subscription_status:'premium' });
    setUsers(us => us.map(x => x.id===u.id?{...x,subscription_status:'premium'}:x));
    toast.success('User upgraded');
  };
  ...
  const statCards = [
    {label:'Total Cars',value:stats.totalCars||0,icon:Car,color:'amber'},
    {label:'Pending Cars',value:stats.pendingCars||0,icon:Clock,color:'orange'},
    {label:'Seller Requests',value:stats.pendingSellerRequests||0,icon:UserCheck,color:'blue'},
    {label:'Stores',value:stats.totalStores||0,icon:Store,color:'emerald'},
    {label:'Total Users',value:stats.totalUsers||0,icon:Users,color:'purple'},
    {label:'Enquiries',value:stats.totalEnquiries||0,icon:MessageSquare,color:'rose'},
  ];
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-6 flex items-center justify-between flex-wrap gap-3">
          <div><h1 className="text-2xl font-bold text-slate-900">Admin Dashboard</h1><p className="text-slate-500">Full control of H-Autos Nigeria</p></div>
          <div className="flex gap-2 flex-wrap">
            ...
            <Badge className="bg-red-100 text-red-700 px-3 py-2"><Shield className="w-3 h-3 mr-1 inline"/>Admin</Badge>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          {statCards.map(({label,value,icon:Icon,color},i)=>(
            <motion.div key={label} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:i*0.05}}>
              <Card><CardContent className="p-4">
                ...
                <p className="text-2xl font-bold">{value}</p><p className="text-xs text-slate-500">{label}</p>
              </CardContent></Card>
            </motion.div>
          ))}
        </div>
        ...
        {(pendingCars.length>0||pendingStores.length>0||pendingReqs.length>0) && (
          <div className="grid md:grid-cols-3 gap-4 mb-6">
            {pendingCars.length>0&&<div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-2"><Clock className="w-4 h-4 text-amber-600"/><p className="text-amber-800 text-sm font-medium">{pendingCars.length} car{pendingCars.length!==1?'s':''} await approval</p></div>}
            ...
            {pendingStores.length>0&&<div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 flex items-center gap-2"><Store className="w-4 h-4 text-emerald-600"/><p className="text-emerald-800 text-sm font-medium">{pendingStores.length} store{pendingStores.length!==1?'s':''} await approval</p></div>}
          </div>
        )}
        ...
        <Tabs defaultValue="seller-requests">
          <TabsList className="mb-6 flex-wrap h-auto gap-1">
            ...
            <TabsTrigger value="ads">Spotlight Ads</TabsTrigger>
          </TabsList>
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 mt-2">
                          <Badge className={req.payment_method==='naira'?'bg-amber-100 text-amber-700':'bg-blue-100 text-blue-700'}>
                            {req.payment_method==='naira'?`₦${Number(req.amount_paid||3000).toLocaleString()} NGN`:`$${req.amount_paid||5} USDT`}
                          </Badge>
          ...
          <TabsContent value="pending-cars">
            <div className="space-y-4">
              {pendingCars.length===0&&<div className="text-center py-12 text-slate-400">No pending listings 🎉</div>}
              {pendingCars.map(car=>(
                <Card key={car.id}>
          ...
          <TabsContent value="all-cars">
            ...
            <div className="bg-white rounded-xl overflow-hidden border border-slate-100">
              <Table>
                <TableHeader><TableRow><TableHead>Car</TableHead><TableHead>Seller</TableHead><TableHead>Price</TableHead><TableHead>Status</TableHead><TableHead>Views</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
                <TableBody>
                  {filtered.map(car=>(
                    <TableRow key={car.id}>
                      <TableCell>
                        <div className="flex items-center gap-3">
                          <img src={car.images?.[0]||'https://images.unsplash.com/photo-1494976388531-d1058494cdd8?w=100'} alt="" className="w-12 h-10 object-cover rounded"/>
                          <div><p className="font-medium text-sm">{car.title}</p><p className="text-xs text-slate-500">{car.state}</p></div>
                        </div>
                      </TableCell>
                      <TableCell className="text-sm">{car.profiles?.full_name||car.seller_name||'Admin'}</TableCell>
                    ...
                    <div className="flex items-start gap-4 justify-between flex-wrap">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-xl bg-slate-100 overflow-hidden flex items-center justify-center shrink-0">
                          {store.logo_url?<img src={store.logo_url} alt="" className="w-full h-full object-cover"/>:<Store className="w-7 h-7 text-slate-400"/>}
                        </div>
                        <div>
                          ...
                          <Badge className={`ml-2 ${store.status==='approved'?'bg-emerald-100 text-emerald-700':store.status==='pending'?'bg-amber-100 text-amber-700':'bg-red-100 text-red-700'}`}>{store.status}</Badge>
                        </div>
                      </div>
                      <div className="flex gap-2 flex-wrap">
                        {store.status==='pending'&&<>
                          <Button size="sm" onClick={()=>approveStore(store)} className="bg-emerald-500 hover:bg-emerald-600 text-white"><CheckCircle2 className="w-4 h-4 mr-1"/>Approve</Button>
                          <Button size="sm" variant="outline" onClick={()=>rejectStore(store)} className="text-red-600 border-red-200"><XCircle className="w-4 h-4 mr-1"/>Reject</Button>
                        </>}
          ...
          <TabsContent value="enquiries">
            <div className="bg-white rounded-xl overflow-hidden border border-slate-100">
              <Table>
                <TableHeader><TableRow><TableHead>Car</TableHead><TableHead>Buyer</TableHead><TableHead>Contact</TableHead><TableHead>Message</TableHead><TableHead>Date</TableHead><TableHead>Action</TableHead></TableRow></TableHeader>
                <TableBody>
                  ...
                  {enquiries.map(enq=>(
                    <TableRow key={enq.id}>
                      <TableCell><div><p className="font-medium text-sm">{enq.car_title||enq.cars?.title}</p><p className="text-xs text-slate-500">{formatPrice(enq.car_price||enq.cars?.price)}</p></div></TableCell>
                      <TableCell><p className="text-sm font-medium">{enq.buyer_name}</p><p className="text-xs text-slate-500">{enq.profiles?.email}</p></TableCell>
                      <TableCell><p className="text-xs text-slate-600">{enq.buyer_phone}</p>{enq.buyer_whatsapp&&<a href={`https://wa.me/${enq.buyer_whatsapp.replace(/\D/g,'')}`} target="_blank" rel="noopener noreferrer" className="text-xs text-green-600 hover:underline">{enq.buyer_whatsapp}</a>}</TableCell>
                      <TableCell><p className="text-sm text-slate-600 max-w-[200px] truncate">{enq.message}</p></TableCell>
                      <TableCell className="text-xs text-slate-500">{new Date(enq.created_at).toLocaleDateString()}</TableCell>
                      <TableCell>
                        <a href={`https://wa.me/${(enq.buyer_whatsapp||enq.buyer_phone||'').replace(/\D/g,'')}?text=${encodeURIComponent(`Hi ${enq.buyer_name}, regarding your enquiry on the ${enq.car_title} on H-Autos...`)}`} target="_blank" rel="noopener noreferrer">
          ...
          <TabsContent value="users">
            <div className="bg-white rounded-xl overflow-hidden border border-slate-100">
              <Table>
                <TableHeader><TableRow><TableHead>User</TableHead><TableHead>WhatsApp</TableHead><TableHead>Seller</TableHead><TableHead>Plan</TableHead><TableHead>Joined</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
                <TableBody>
                  {users.map(u=>(
                    <TableRow key={u.id}>
                      <TableCell><div><p className="font-medium text-sm">{u.full_name||'—'}</p><p className="text-xs text-slate-500">{u.email}</p></div></TableCell>
                      ...
                      <TableCell><Badge className={u.seller_active?'bg-emerald-100 text-emerald-700':'bg-slate-100 text-slate-600'}>{u.seller_active?'Active':'—'}</Badge></TableCell>
                      <TableCell><Badge className={u.subscription_status==='premium'?'bg-amber-100 text-amber-700':'bg-slate-100 text-slate-600'}>{u.subscription_status==='premium'?'Premium':'Free'}</Badge></TableCell>
                      <TableCell className="text-xs text-slate-500">{u.created_at?.split('T')[0]}</TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          {!u.seller_active&&<Button size="sm" variant="outline" onClick={()=>activateSeller(u)} className="h-8 text-xs gap-1"><UserCheck className="w-3 h-3"/>Activate Seller</Button>}
                          {u.subscription_status!=='premium'&&<Button size="sm" variant="outline" onClick={()=>upgradeUser(u)} className="h-8 text-xs gap-1"><Crown className="w-3 h-3"/>Premium</Button>}
                        </div>
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Make *</Label><Select value={listForm.make} onValueChange={v=>setListForm(p=>({...p,make:v}))}><SelectTrigger className="mt-1"><SelectValue placeholder="Make"/></SelectTrigger><SelectContent>{CAR_MAKES.map(m=><SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent></Select></div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Fuel Type</Label><Select value={listForm.fuel_type} onValueChange={v=>setListForm(p=>({...p,fuel_type:v}))}><SelectTrigger className="mt-1"><SelectValue/></SelectTrigger><SelectContent>{['petrol','diesel','electric','hybrid','CNG'].map(f=><SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent></Select></div>
            ...
            <div><Label>Assign to Store (optional)</Label><Select value={listForm.store_id} onValueChange={v=>setListForm(p=>({...p,store_id:v}))}><SelectTrigger className="mt-1"><SelectValue placeholder="No store (admin listing)"/></SelectTrigger><SelectContent><SelectItem value="">No store</SelectItem>{stores.filter(s=>s.status==='approved').map(s=><SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select></div>
            <div><Label>Seller WhatsApp</Label><Input value={listForm.seller_whatsapp} onChange={e=>setListForm(p=>({...p,seller_whatsapp:e.target.value}))} className="mt-1" placeholder="+2348146730044"/></div>
            <div><Label>Custom Title (optional)</Label><Input value={listForm.title} onChange={e=>setListForm(p=>({...p,title:e.target.value}))} className="mt-1" placeholder={`${listForm.year} ${listForm.make} ${listForm.model}`}/></div>
            ...
            <div>
              ...
              <label className="mt-2 flex flex-col items-center h-24 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:border-amber-400 hover:bg-amber-50 transition-all justify-center">
                ...
                {uploadingCarImages?<Loader2 className="w-6 h-6 animate-spin text-amber-500"/>:<><ImageIcon className="w-6 h-6 text-slate-400 mb-1"/><p className="text-xs text-slate-500">Upload photos</p></>}
              </label>
              {listForm.images.length>0&&<div className="grid grid-cols-5 gap-2 mt-2">{listForm.images.map((url,i)=><div key={i} className="relative aspect-square rounded overflow-hidden group"><img src={url} alt="" className="w-full h-full object-cover"/><button onClick={()=>setListForm(p=>({...p,images:p.images.filter((_,j)=>j!==i)}))} className="absolute top-0.5 right-0.5 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center opacity-0 group-hover:opacity-100 text-[10px]"><X className="w-2.5 h-2.5"/></button></div>)}</div>}
            </div>
