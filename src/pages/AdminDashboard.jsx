...
export default function AdminDashboard() {
  ...
  const [listForm, setListForm] = useState({
    ...
    condition: 'nigerian_used', description: '', images: [], seller_name: 'HP-Autos Admin',
  ...
  const approveCar = async (car) => {
    ...
    await sendEmailNotification({ to: car.seller_email || car.profiles?.email, subject: `✅ Listing Approved – ${car.title}`, html: `<h2>${car.title} is now live on HP-Autos!</h2><p>Buyers can now see your listing.</p>` });
  ...
  const approveSellerReq = async (req) => {
    ...
    await sendEmailNotification({ to: req.profiles?.email, subject: '🎉 Seller Access Approved!', html: `<h2>You're now an active seller on HP-Autos!</h2><p>Your ${SELLER_DURATION_DAYS}-day seller subscription is active. WhatsApp admin to submit cars: wa.me/${ADMIN_WHATSAPP}</p>` });
  ...
  const approveStore = async (store) => {
    ...
    await sendEmailNotification({ to: store.profiles?.email || store.email, subject: `🏪 ${store.name} is live on HP-Autos!`, html: `<h2>${store.name} approved!</h2><p>Your store is now live. Admin will list cars for your store.</p>` });
  ...
  // Car images upload — tries Supabase storage, shows helpful error if bucket missing
  const handleCarImages = async (e) => {
    const files = Array.from(e.target.files);
    setUploadingImgs(true);
    for (const f of files) {
      try {
        const url = await uploadFile(f, 'cars');
        setListForm(p => ({ ...p, images: [...p.images, url] }));
      } catch (err) {
        ...
        if (err?.message?.includes('Bucket not found') || err?.statusCode === 400) {
          ...
          toast.error(`Failed to upload ${f.name}. Try pasting an image URL instead.`);
        }
      }
    }
  ...
  const handleAdminList = async () => {
    if (!listForm.make || !listForm.model || !listForm.price) {
      toast.error('Make, model and price are required'); return;
    }
    try {
      ...
      const carData = {
        ...
        year: parseInt(listForm.year) || new Date().getFullYear(),
        price: parseFloat(listForm.price),
        mileage: listForm.mileage ? parseFloat(listForm.mileage) : null,
        fuel_type: listForm.fuel_type || 'petrol',
        transmission: listForm.transmission || 'automatic',
        condition: listForm.condition || 'nigerian_used',
        country: listForm.country || 'NG',
        state: listForm.state || '',
        city: listForm.city || '',
        description: listForm.description || '',
        images: listForm.images || [],
        seller_name: listForm.seller_name || 'HP-Autos Admin',
        seller_phone: listForm.seller_phone || '',
        seller_whatsapp: listForm.seller_whatsapp || '+2348146730044',
        status: 'approved',
        is_verified: true,
        is_featured: false,
        views: 0,
        ...
        store_id: listForm.store_id || null,
        features: [],
      };
      await createCar(carData);
      toast.success('Car listed successfully!');
      setShowListCar(false);
      setListForm({ make:'',model:'',year:new Date().getFullYear(),price:'',state:'',city:'',condition:'nigerian_used',description:'',images:[],seller_name:'HP-Autos Admin',seller_phone:'',seller_whatsapp:'+2348146730044',fuel_type:'petrol',transmission:'automatic',mileage:'',title:'',store_id:'',country:'NG' });
      loadAll();
    } catch (e) {
      console.error('List car error:', e);
      toast.error(e?.message || 'Failed to list car. Check console for details.');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 py-6 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2 flex-wrap w-full sm:w-auto">
            <Button onClick={() => setShowRevModal(true)} variant="outline" className="gap-1.5 sm:gap-2 text-emerald-600 border-emerald-200 text-xs sm:text-sm h-9 px-2.5 sm:px-4"><DollarSign className="w-4 h-4" /><span className="hidden sm:inline">Log Revenue</span><span className="sm:hidden">Revenue</span></Button>
            <Button onClick={() => setShowAdModal(true)} variant="outline" className="gap-1.5 sm:gap-2 text-xs sm:text-sm h-9 px-2.5 sm:px-4"><Megaphone className="w-4 h-4" /><span className="hidden sm:inline">New Ad</span><span className="sm:hidden">Ad</span></Button>
            <Button onClick={() => setShowListCar(true)} className="bg-amber-500 hover:bg-amber-600 text-white gap-1.5 sm:gap-2 text-xs sm:text-sm h-9 px-2.5 sm:px-4"><Plus className="w-4 h-4" />List Car</Button>
            <Badge className="bg-red-100 text-red-700 px-2.5 sm:px-3 py-2 text-xs sm:text-sm"><Shield className="w-3 h-3 mr-1 inline" />Admin</Badge>
          </div>
        </div>
      </div>
                ...
                <TableBody>
                  ...
                  {enquiries.map(enq => (
                    <TableRow key={enq.id}>
                      ...
                      <TableCell>
                        <a href={`https://wa.me/${(enq.buyer_whatsapp || enq.buyer_phone || '').replace(/\D/g, '')}?text=${encodeURIComponent(`Hi ${enq.buyer_name}, regarding your enquiry on the ${enq.car_title} on HP-Autos Nigeria...`)}`} target="_blank" rel="noopener noreferrer">
      ...
      <Dialog open={showListCar} onOpenChange={setShowListCar}>
        <DialogContent className="max-w-2xl w-[95vw] sm:w-full max-h-[90vh] overflow-y-auto rounded-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div><Label>Price ({(COUNTRIES.find(c => c.code === (listForm.country || 'NG'))?.currency) || '₦'}) *</Label><Input type="number" value={listForm.price} onChange={e => setListForm(p => ({ ...p, price: e.target.value }))} className="mt-1" placeholder="5000000" /></div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><Label>Country *</Label><CountrySelector value={listForm.country || 'NG'} onChange={v => setListForm(p => ({ ...p, country: v, state: '' }))} className="mt-1" /></div>
              <div><Label>State / Region *</Label><RegionSelector countryCode={listForm.country || 'NG'} value={listForm.state} onChange={v => setListForm(p => ({ ...p, state: v }))} className="mt-1" /></div>
            </div>
            <div>
              <Label>City</Label><Input value={listForm.city} onChange={e => setListForm(p => ({ ...p, city: e.target.value }))} className="mt-1" placeholder="e.g. Ikeja, Houston, Manchester" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div><Label>Assign to Store (optional)</Label><Select value={listForm.store_id || 'none'} onValueChange={v => setListForm(p => ({ ...p, store_id: v === 'none' ? '' : v }))}><SelectTrigger className="mt-1"><SelectValue placeholder="No store (admin listing)" /></SelectTrigger><SelectContent><SelectItem value="none">No store</SelectItem>{(stores || []).filter(s => s.status === 'approved').map(s => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select></div>
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                <label className="flex flex-col items-center h-20 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:border-amber-400 hover:bg-amber-50 transition-all justify-center">
                  <input type="file" multiple accept="image/*" onChange={handleCarImages} className="hidden" />
                  {uploadingImgs ? <Loader2 className="w-5 h-5 animate-spin text-amber-500" /> : <><ImageIcon className="w-5 h-5 text-slate-400 mb-1" /><p className="text-xs text-slate-500">Click to upload photos</p></>}
                </label>
                <div className="flex gap-2">
                  <Input
                    ...
                    onKeyDown={e => {
                      if (e.key === 'Enter' && e.target.value.startsWith('http')) {
                        setListForm(p => ({ ...p, images: [...p.images, e.target.value] }));
                        e.target.value = '';
                      }
                    }}
                  />
                  <Button type="button" variant="outline" size="sm" className="shrink-0 text-xs"
                    onClick={(e) => {
                      ...
                      if (input?.value?.startsWith('http')) {
                        setListForm(p => ({ ...p, images: [...p.images, input.value] }));
                        input.value = '';
                      }
                    }}>Add URL</Button>
                </div>
                <p className="text-xs text-slate-400">Tip: You can also paste URLs from Google Drive, Cloudinary, or any public image host.</p>
              </div>
              {listForm.images.length > 0 && (
                <div className="grid grid-cols-5 gap-2 mt-2">
                  {listForm.images.map((url, i) => (
                    <div key={i} className="relative aspect-square rounded overflow-hidden group">
                      <img src={url} alt="" className="w-full h-full object-cover" onError={e => e.target.src='https://images.unsplash.com/photo-1494976388531-d1058494cdd8?w=100'} />
                      <button onClick={() => setListForm(p => ({ ...p, images: p.images.filter((_, j) => j !== i) }))} className="absolute top-0.5 right-0.5 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center opacity-0 group-hover:opacity-100 text-[10px]">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
            ...
            <Button onClick={handleAdminList} className="w-full bg-amber-500 hover:bg-amber-600 text-white h-12 text-base font-semibold">
              Post Car Listing
            </Button>
          </div>
        </DialogContent>
