...
function SpotlightBanner() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -60 }} className="fixed top-7 sm:top-8 left-0 right-0 z-[55] bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 sm:py-3 flex items-center gap-2 sm:gap-4">
        {current.image_url && <img src={current.image_url} alt="Ad" className="h-8 w-12 sm:h-10 sm:w-16 object-cover rounded shrink-0 hidden xs:block" />}
...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* 24/7 rental marquee — always on top, above header */}
      <div className="fixed top-0 left-0 right-0 z-[60]">
        <RentalBanner />
      </div>
      <SpotlightBanner />
      <header className={`fixed top-7 sm:top-8 left-0 right-0 z-50 transition-all duration-300 ${hBg}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              {isAuthenticated ? (
                ...
                <div className="hidden md:flex gap-2">
                  <Button variant="outline" onClick={() => setAuthOpen(true)} className="border-amber-400 text-amber-600 hover:bg-amber-50 font-semibold h-9 px-4">Sign In</Button>
                  <Button onClick={() => setAuthOpen(true)} className="bg-amber-500 hover:bg-amber-600 text-white h-9 px-4 font-semibold shadow-sm">Register Free</Button>
                </div>
              )}
      ...
      <main className="pt-[4.75rem] sm:pt-[5.5rem] md:pt-[6.5rem]">{children}</main>
