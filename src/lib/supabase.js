...
export const updateAd = async (id, updates) => { const { data, error } = await supabase.from('spotlight_ads').update(updates).eq('id', id).select().single(); if (error) throw error; return data; };
...
export const notifyAdminWhatsApp = (message) => {
...
export const getAdminStats = async () => {
  const [cars, users, stores, requests, enquiries] = await Promise.all([
    supabase.from('cars').select('id, status, views'),
    supabase.from('profiles').select('id', { count: 'exact', head: true }),
    supabase.from('stores').select('id, status'),
    supabase.from('seller_requests').select('id, status'),
