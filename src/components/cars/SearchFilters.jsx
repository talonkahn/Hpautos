      ...
      <div className="flex gap-2 sm:gap-3">
        <div className="relative flex-1 min-w-0">
          ...
          <Input
            ...
            className="pl-10 bg-white h-10 sm:h-11 text-sm"
          />
        </div>
        <Button
          variant="outline"
          onClick={() => setOpen(o => !o)}
          className={`h-10 sm:h-11 gap-1.5 sm:gap-2 shrink-0 px-2.5 sm:px-4 text-sm ${open ? 'border-amber-500 text-amber-600 bg-amber-50' : ''}`}
        >
          <SlidersHorizontal className="w-4 h-4" />
          <span className="hidden sm:inline">Filters</span>
          {activeCount > 0 && (
            <Badge className="bg-amber-500 text-white text-[10px] px-1.5 py-0 ml-0.5">{activeCount}</Badge>
          )}
        </Button>
        {activeCount > 0 && (
          <Button variant="ghost" onClick={clear} className="h-10 sm:h-11 gap-1 text-slate-500 shrink-0 px-2 sm:px-3 text-sm">
