      ...
      <Card className="group overflow-hidden bg-white border-0 shadow-sm hover:shadow-xl transition-all duration-500">
        ...
        <CardContent className="p-5">
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100">
            <p className="text-2xl font-bold text-slate-900">{formatPrice(car.price, car.country)}</p>
            <div className="flex items-center text-sm text-slate-500 gap-1">{car.country && getCountry(car.country) ? <span>{getCountry(car.country).flag}</span> : <MapPin className="w-4 h-4"/>}<span>{car.state || car.country}</span></div>
          </div>
        </CardContent>
      </Card>
    </motion.div>
